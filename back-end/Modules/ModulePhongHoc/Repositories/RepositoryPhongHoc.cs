...
namespace Education_assistant.Modules.ModulePhongHoc.Repositories
{
    public class RepositoryPhongHoc : RepositoryBase<PhongHoc>, IRepositoryPhongHoc
    {
        ...
        public async Task<PagedListAsync<PhongHoc>> GetAllPhongHocAsync(int page, int limit, string search, string sortBy, string sortByOrder, int? loaiPhong, int? trangThai, string? toaNha)
        {
            ...
            {
                query = query.Where(item => item.ToaNha.ToUpper() == toaNha.ToUpper());
            }
            if (loaiPhong.HasValue && loaiPhong != 0)
            {
                query = query.Where(item => item.LoaiPhongHoc == loaiPhong);
            }
        ...
        {
            return await _context.PhongHocs!.AsNoTracking().Where(p => tenPhongs.Contains(p.TenPhong.ToUpper())).Select(item => item.TenPhong.ToUpper()).ToListAsync();
        }

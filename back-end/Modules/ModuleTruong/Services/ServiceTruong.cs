...
public class ServiceTruong : IServiceTruong
{
    ...
    public async Task DeleteAsync(Guid id)
    {
        if (id == Guid.Empty)
        {
            throw new TruongBadRequestException($"Trường với {id} không được bỏ trống!");
        }
    ...
    public async Task UpdateAsync(Guid id, RequestUpdateTruongDto request)
    {
        if (id != request.Id)
        {
            throw new TruongBadRequestException($"Id và Id của trường không giống nhau!");
        }
        if (request is null)
        {
            throw new TruongBadRequestException($"Thông tin trường không đầy đủ!");
        }
        var truong = await _repositoryMaster.Truong.GetTruongByIdAsync(id, false);
        if (truong is null)
        {

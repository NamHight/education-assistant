...
public class RepositoryGiangVien : RepositoryBase<GiangVien>, IRepositoryGiangVien
{
    ...
    {
        return await FindByCondition(gv => gv.Email.Equals(email), trackChanges).Include(g => g.TaiKhoan)
            .FirstOrDefaultAsync();
    }
    ...
    {
        return await FindByCondition(item => item.KhoaId == khoaId && item.DeletedAt == null, false)
    ...
    {
        return await FindByCondition(item => item.TaiKhoanId == taiKhoanId, false).Include(item => item.BoMon)
            .IgnoreQueryFilters().Include(item => item.Khoa).Include(item => item.TaiKhoan).FirstOrDefaultAsync();
    }

    public async Task<IEnumerable<GiangVien>?> GetAllGiangVienByBoMonAsync(Guid boMonId)
    {
        return await FindByCondition(item => item.BoMonId == boMonId && item.DeletedAt == null, false)
            .Include(item => item.BoMon).IgnoreQueryFilters().ToListAsync();
    }

    public async Task<IEnumerable<GiangVien>?> GetAllGiangVienNoPageAsync()
    {
        return await FindByCondition(
                item => item.TrangThai == (int)TrangThaiGiangVienEnum.DANG_CONG_TAC && item.DeletedAt == null, false)
            .IgnoreQueryFilters().ToListAsync();
    }
    ...
    public async Task<int> GetAllTrangThainDangCongTacAsync(Guid? khoaId)
    {
        var query = _context.GiangViens!
            ...
            .IgnoreQueryFilters()
            .AsQueryable();
        if (khoaId.HasValue && khoaId != Guid.Empty) query = query.Where(item => item.KhoaId == khoaId);

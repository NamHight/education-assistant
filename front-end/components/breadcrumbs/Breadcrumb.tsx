...
const Breadcrumb = () => {
    ...
    const handleRenderFirstBreadcrumb = useMemo(() : ReactNode => {
      const splitPath = path.split('/').filter(Boolean)[0];
        if (path === '/') {
            return <Link href={`/${splitPath}`} className="relative text-[14px] flex gap-1 items-center text-black dark:text-white
    ...
    before:w-0 hover:before:w-full before:transition-all before:duration-400">
       ...
       <Typography sx={(theme) => ({
        ...
        return <Link href={`/${splitPath}`} className="relative text-[14px] flex gap-1 items-center
    before:absolute before:left-0 before:bottom-0 before:h-[2px] before:bg-blue-500
    before:w-0 hover:before:w-full before:transition-all before:duration-400">
      <HomeOutlinedIcon sx={(theme) => ({
        color: theme.palette.mode === 'dark' ? 'white !important' : 'black !important',
    ...
    const handleRenderSecondBreadcrumb = useMemo(() : ReactNode => {
      const splitPath = path.split('/').filter(Boolean)[1];
       return (!splitPath || !breadcrumbTranslations[splitPath]) ? null : <Link href={`/${splitPath}`} className="relative text-[14px] flex gap-1 items-center text-black dark:text-white
    before:absolute before:left-0 before:bottom-0 before:h-[2px] before:bg-blue-500
    before:w-0 hover:before:w-full before:transition-all before:duration-400">
          <AddCircleOutlineOutlinedIcon sx={(theme) => ({
        color: theme.palette.mode === 'dark' ? 'white !important' : 'black !important',
      })} className="!h-4 !w-4"/>
          <Typography sx={(theme) => ({
            color: theme.palette.mode === 'dark' ? 'white !important' : 'black !important',
  ...
  return (
     <Box className="relative">
      <Breadcrumbs
        ...
        separator={<NavigateNextIcon fontSize="small" sx={(theme) => ({
          color: theme.palette.mode === 'dark' ? 'white !important' : 'black !important',
          fontSize: '1rem'
        })} />}

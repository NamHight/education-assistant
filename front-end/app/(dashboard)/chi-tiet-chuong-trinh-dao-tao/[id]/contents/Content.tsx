...
const Content = ({ initialData, id, anotherData }: IContentProps) => {
  ...
  const { data } = useQuery({
    queryKey: ['chi-tiet-chuong-trinh-dao-tao', { id: id }],
    queryFn: async () => {
      const result = await ChitietChuongTrinhDaoTaoService.getChiTietChuongTrinhDaoTaoById(id);
      return result;
    },
  ...
  const mutationUpdate = useMutation({
    mutationFn: async (data: FormData) => {
      const result = await ChitietChuongTrinhDaoTaoService.updateChiTietChuongTrinhDaoTao(id, data);
      return result;
    },
    onSuccess: async (data: any) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['chi-tiet-chuong-trinh-dao-tao-list'], exact: false });

    ...
    const {data} = useQuery({
     ...
     const schema = useMemo(() => {
        return yup.object().shape({
          ...
          LoaiPhongHoc: yup.object().required('Loại phòng học không được để trống').typeError('Loại phòng học không được để trống'),
          TrangThaiPhongHoc: yup.object().required('Trạng thái phòng học không được để trống').typeError('Trạng thái phòng học không được để trống'),
        });
      }, [data]);
      useEffect(() => {
        if(open && !!id){
      ...
      useEffect(() => {
        if (data) {
          reset({
            ...
            LoaiPhongHoc: data?.loaiPhongHoc ? 
            LoaiPhongHoc.find((item) => item.id === data.loaiPhongHoc)
            : null,
     ...
     <Dialog
      ...
      >
        <DialogTitle id="alert-dialog-title" className="text-lg font-semibold text-blue-600">
          Thay đổi phòng học
          <Typography component={'span'} className="!ml-1 !text-lg !font-semibold !text-blue-600">{name}</Typography>
        </DialogTitle>
        <DialogContent>

...
const Content: FC<ContentProps> = ({ initialData, anotherData }) => {
  ...
  const mutationCreate = useMutation({
    mutationFn: async (formData: any) => {
      const response = await ChuongTrinhDaoTaoService.createChuongTrinhDaoTao(formData);
      return response;
    },
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['chuong-trinh-dao-tao-list'],
        exact: false
      });
  ...
  console.log('initialData', anotherData.nganhs);
  return (
